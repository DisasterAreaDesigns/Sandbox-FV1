...
The encoding is a strict superset: it uses bit 20 of `RDA`/`WRA`/`WRAP` and bit 5 of `RMPA`, neither of which the FV-1 defines and neither of which is set by any program we have found.  An ordinary FV-1 binary means exactly the same thing whether or not the pragma is on, so there is no mode flag for a loader to get wrong.  What this does mean is that **an extended program will not run on an FV-1** - the parts of it that need the extra bits will read the wrong addresses.  Without the pragma, `RMPAX` and `POT3`-`POT5` are refused by name rather than failing as undefined labels.
