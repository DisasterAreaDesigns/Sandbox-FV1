...
We've also added an FV-1 simulator to the web app.  The assembled program runs through a software model of the chip at its native 32.768 kHz, so you can audition an algorithm on a test tone, an audio file, or live input before you ever program the hardware.  The three pot controls are on screen, and each successful build reloads into the simulator while it plays.  Two indicator lamps follow `REG30` and `REG31`, as the FV-2040 firmware does.  The simulator models accumulator saturation and the FV-1's companded delay memory, and its LFO rates and depths follow the equations in Spin's application note AN-0001.  Chorus interpolation is approximated, so always confirm a design on real hardware.

The simulator can also be played from a MIDI controller plugged into your computer.  Press **Enable MIDI** in the simulator panel, allow the browser's MIDI prompt, and pick an input and channel.  `CC50` through `CC55` sweep POT0 through POT5, and `CC102` switches the effect, with `0-63` bypassed and `64-127` engaged.  MIDI moves the same controls the sliders do, at the full 128 steps a control change carries, so a program cannot tell the difference between a fader and a mouse.  Note that the Sandbox pedal itself has no MIDI input - this drives the simulator only.

## The Extended Instruction Set
A program that begins with a `#extended` line is assembled for an extended version of the FV-1 instruction set, as implemented by the FV-2040 and by asfv1-extended.  It adds 65536 words of delay memory instead of 32768, three more pot registers at `POT3`-`POT5`, and `RMPAX`, which reads `ADDR_PTR` as `ACC[22:7]` so a single pointer sweep reaches the whole tank.

It also adds `RAND reg, coeff`, which fills a register with `coeff` times a uniform draw over [-1, 1) and leaves the accumulator alone - the only register write in the instruction set that does - so `rand noise, 1.0` is full-scale white and `rand dither, 0.002` is a dither, each in one instruction where an FV-1 program spends four or five building an LFSR out of `AND`/`XOR`/`SOF`.

And it doubles the LFOs, from `SIN0`-`SIN1` and `RMP0`-`RMP1` to four of each.  `SIN2`, `SIN3`, `RMP2` and `RMP3` take their rate and range registers from `0x08`-`0x0f`, the block the FV-1 leaves empty, and behave exactly as the first four do: `WLDS`/`WLDR` write them, `WRAX` writes them, `RDAX` reads them, and every `CHO` flag means the same thing on `SIN3` as it does on `SIN0`.  Four ramps is four pitch voices.

The encoding is a strict superset: it uses bit 20 of `RDA`/`WRA`/`WRAP`, bit 5 of `RMPA`, bit 23 of `CHO`, bit 8 of `JAM`, bit 31 of `WLDS`/`WLDR` and opcode 21 for `RAND` - none of which the FV-1 defines and none of which is set by any program we have found.  An ordinary FV-1 binary means exactly the same thing whether or not the pragma is on, so there is no mode flag for a loader to get wrong.  What this does mean is that **an extended program will not run on an FV-1** - the parts of it that need the extra bits will read the wrong addresses.  Without the pragma, `RMPAX`, `RAND`, `POT3`-`POT5`, `SIN2`, `SIN3`, `RMP2`, `RMP3` and the eight new LFO registers are refused by name rather than failing as undefined labels.

The simulator models all of it.  The `POT3`-`POT5` sliders appear when the loaded build declares the pragma and are hidden again when it does not, and the maximum delay time shown beside the crystal follows whichever tank is in use.
